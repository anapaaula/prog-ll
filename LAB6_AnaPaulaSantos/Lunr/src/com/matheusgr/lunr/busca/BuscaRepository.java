...
public class BuscaRepository {
	...
	private List<HistoricoBusca> hitoricoDeBuscas;
	...
	public BuscaRepository() {
		this.hitoricoDeBuscas = new ArrayList<>();
	 ...
	 */
	public void adicionaBuscaNoHistorico(Busca busca, DocumentoDTO[] documentos) {
		String[] ids = Stream.of(documentos)
			.map(DocumentoDTO::getId)
			.collect(Collectors.toList())
			.toArray(new String[] {});
		this.hitoricoDeBuscas.add(new HistoricoBusca(busca, ids));
	}
	...
	public HistoricoBusca recuperar(int numero) {
		...
		if (numero > this.hitoricoDeBuscas.size()) {
			throw new IllegalArgumentException("Histórico inexistente: " + numero);
		}
		return this.hitoricoDeBuscas.get(numero);
	}
