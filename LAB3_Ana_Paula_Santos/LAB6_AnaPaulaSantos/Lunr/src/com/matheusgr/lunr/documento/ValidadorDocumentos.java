...
class ValidadorDocumentos extends ValidadorPadrao {
	...
	public void validacao(Set<Documento> docs) {
		Objects.requireNonNull(docs, "Conteúdo não pode ser nulo");
		
		for(Documento doc: docs) {
			if(doc != null) {
				 validacao(doc.getId(), doc.getTexto());
				 return;
			}
			
		}
		throw new IllegalArgumentException("Conjunto de documentos vazia!");
	
	}
	
	public void validacao(Map<String,Documento> documentos) {
		Objects.requireNonNull(documentos, "Conteúdo não pode ser nulo");
		
		for (Entry<String, Documento> entry : documentos.entrySet()) {
	         if(entry.getKey()!= null && !entry.getKey().isBlank()) {
	        	 if(entry.getValue() != null) {
	        		 validacao(entry.getValue().getId(), entry.getValue().getTexto());
	        		 return;
	        	 }
	         }
		}
		throw new IllegalArgumentException("Mapa de documentos sem elementos válidos!");
